...
def tkinter_run():
    ...
    Radiobutton(window, text='Vortex', variable=radio_var, value=3, justify='left', anchor='w').grid(
        column=0, row=7, sticky=W)
    Label(window, text='Period').grid(column=0, row=7)
    vortex_period_val = IntVar(value=SETTINGS.get('VORTEX_PERIOD', 14))
    ...
    Radiobutton(window, text='Bollinger Bands', variable=radio_var, value=6, justify='left', anchor='w').grid(
        column=0, row=10, sticky=W)
    Label(window, text='Period').grid(column=0, row=10)
    bb_period_val = IntVar(value=SETTINGS.get('BB_PERIOD', 20))
    ent_bb_period = Entry(window, width=2, justify='right', textvariable=bb_period_val)
    ent_bb_period.grid(column=0, row=10, sticky=E)
    ...
    if SETTINGS.get('RSI_ENABLED', False) is True:
        chk_rsi.select()
    chk_rsi.grid(column=0, row=11, sticky=W)
    ...
    rsi_upper_drop.grid(column=0, row=12)
    ...
    ent_rsi_upper.grid(column=0, row=12, sticky=E)
    ...
    lbl_rsi_put.grid(column=0, row=13, sticky=W)
    ...
    rsi_lower_drop.grid(column=0, row=13)
    ...
    ent_rsi_lower.grid(column=0, row=13, sticky=E)
    ...
    chk_supertrend.grid(column=0, row=14, sticky=W)
    ...
    lbl_supertrend_period.grid(column=0, row=15, sticky=W)
    ...
    ent_supertrend_period.grid(column=0, row=15, sticky=E)
    ...
    def run():
        ...
        if not validate_int(ent_cci_period.get(), 2, 99):
            ...
            return
        if not validate_int(ent_bb_period.get(), 2, 99):
            error_variable.set('Bollinger Bands period: should be number 2-99')
            return
        ...
        save_settings(
            ...
            BB_PERIOD=int(ent_bb_period.get()),
